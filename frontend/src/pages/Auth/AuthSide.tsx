export default function AuthSide({ side }: { side: "left" | "right" }) {
  ...
  return (
    <section className="relative hidden h-full flex-col justify-between overflow-hidden bg-[#0f0e0d] px-10 py-6 lg:flex xl:px-14">
      <div
        className={`pointer-events-none absolute -top-32 h-96 w-96 rounded-full bg-[#ffb4a1]/10 blur-3xl ${
          isLeft ? "-left-32" : "-right-32"
        }`}
      />
      <div
        className={`pointer-events-none absolute top-1/2 h-125 w-125 -translate-y-1/2 rounded-full bg-[#bd8718]/15 blur-[55px] ${
          isLeft ? "left-[15%]" : "right-[15%]"
        }`}
      />

      <div
        className={`absolute top-0 h-full w-10 ${isLeft ? "bg-linear-to-l right-0" : "bg-linear-to-r left-0 "} from-[#141312] to-[#0f0e0d]`}
      />
